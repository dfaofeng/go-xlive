...
func (h *SubscriptionHub) Broadcast(sessionID string, event *realtimev1.SessionEvent) {
	...
	if !ok || len(subscribers) == 0 {
		h.mu.RUnlock()
		// h.logger.Debug("无需广播：无订阅者", zap.String("session_id", sessionID))
		return
	}
...
// --- 移除 BroadcastEvent 方法 (因为 Hub 的 Broadcast 方法已导出) ---
...
func (s *RealtimeService) SubscribeSessionEvents(req *realtimev1.SubscribeSessionEventsRequest, stream realtimev1.RealtimeService_SubscribeSessionEventsServer) error {
	...
	s.logger.Info("收到新的事件流订阅请求", zap.String("sid", sessionID))
	// TODO: 认证
...
// sendInitialSnapshot (可选，实现不变)
// func (s *RealtimeService) sendInitialSnapshot(...) { /* ... */ }
