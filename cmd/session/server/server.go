...
func NewGrpcServer(logger *zap.Logger, port int, sessionSvc sessionv1.SessionServiceServer) (*GrpcServer, error) {
	if port == 0 {
		port = 50053
	} // 默认端口
	...
	// 注册 Session 服务
	sessionv1.RegisterSessionServiceServer(s, sessionSvc)
	reflection.Register(s)

	// 注册健康检查服务
	healthServer := health.NewServer()
	grpc_health_v1.RegisterHealthServer(s, healthServer)
	healthServer.SetServingStatus(sessionv1.SessionService_ServiceDesc.ServiceName, grpc_health_v1.HealthCheckResponse_SERVING)

	logger.Info("Session gRPC 服务准备就绪", zap.String("address", lis.Addr().String()))
...
func (gs *GrpcServer) Shutdown(ctx context.Context) {
	...
	gs.HealthServer.SetServingStatus(sessionv1.SessionService_ServiceDesc.ServiceName, grpc_health_v1.HealthCheckResponse_NOT_SERVING)
